...
for n = 1:numIterations
    ...
    [YTrain,~,H, A]= runMultiLayer(XTrain, Wout, Vout);
    
    
    grad_v = 2 * H' * (YTrain - DTrain) / NTrain; % Gradient for the output layer
    
    
    %Vout2 = Vout;
    %Vout2(end,:) = [];
    grad_w = (2 * (Vout(1:end-1,:) * (YTrain - DTrain)'.*(tanhprim(A))') * XTrain / NTrain)'; % And the input layer
